...
//GLOBAL VARIABLES
bool global_reversePolishNotation = false;
//

status_t parseText(char* pInputText, uint8_t* pOutputTokens, uint8_t* pOperatorStack) {
    if(*pInputText >= 'a' && *pInputText <= 'z') {
        /* process parser directives */
        if(strcmp(pInputText, "rpolish") == 0) {
            global_reversePolishNotation = true;
            echo("Switched to Reverse Polish Notaion");
            return SUCCESS;
        } else if (strcmp(pInputText, "infix") == 0) {
            global_reversePolishNotation = false;
            echo("Switched to Infix Notaion");
            return SUCCESS;
        } else if (strcmp(pInputText, "quit") == 0 || strcmp(pInputText, "exit") == 0) {
            return QUIT;
        }
    }
    if(global_reversePolishNotation) {
        parseReversePolishText(pInputText, pOutputTokens, pOperatorStack);
    } else {
        parseInfixText(pInputText, pOutputTokens, pOperatorStack);
    }
    return SUCCESS;
}
